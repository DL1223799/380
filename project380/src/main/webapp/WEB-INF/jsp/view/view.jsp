            ...
            <c:when test="${!empty pollings}" >
                ...
                <div style="border-style:solid; margin-top: 1px;">
                    <c:forEach items="${pollings}" var="polling">
                        ...
                        <security:authorize access="hasRole('ADMIN')">
                            <a href="<c:url value='/user/delete/${course.id}/Polling/${polling.id}' />" >[Delete]</a> total:${numboptions[n]} [<a href="<c:url value='/course/votehistory/${polling.id}'/>">Vote History</a>]
                        </security:authorize>
                        <br/>
